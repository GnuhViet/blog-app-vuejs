    ...
    idArticle(newValue) {
      ...
      axios
        ...
        .then((res) => {
          ...
          const { id, title, thumbnail, shortDescription, content, categories } = articleData;
          this.inputData.id = id;
          this.inputData.title = title;
          this.inputData.thumbnail = thumbnail;
          this.inputData.shortDescription = shortDescription;
          this.inputData.content = content;

          this.categoriesChecked = new Array(categories.length).fill(0).map(() => {
            return articleData.categories[i].id;
          });
        })
        .catch((err) => {
          console.log(err);
